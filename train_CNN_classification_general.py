...
all_correct = 0
all_total = 0
all_array = np.zeros((len(emotions), len(emotions)))
...
overall_accuracy = 100 * all_correct / all_total if all_total > 0 else 0
print(f'Overall Correct: {all_correct}, Overall accuracy: {overall_accuracy:.3f}%')

array_norm = np.around(all_array.astype('float') / np.sum(all_array, axis=1)[:, None], decimals=3)
...
for i, emotion in enumerate(target_names):
    true_positives = all_array[i, i]
    false_positives = np.sum(all_array[:, i]) - true_positives
    false_negatives = np.sum(all_array[i, :]) - true_positives
...
# plt.show()
plt.savefig(f"results/all_cnn_confusion_matrix.png")
