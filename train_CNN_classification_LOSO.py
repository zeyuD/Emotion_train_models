...
from utils.data_loader import load_image_train_data_loso, load_image_test_data_loso
...
Tasks did by users
u0: lw
u2: lw, drink, knock, s, star, stir, throw, tri, wave
u3: lw
u4: lw
u5: lw
u7: lw, drink, knock, s, star, stir, throw, tri, wave
u8: lw, drink, knock, s, star, stir, throw, tri, wave
u9: lw, drink, knock, s, star, stir, throw, tri, wave
u10: lw
u11: lw
"""
...
task = tasks[0]
posture = postures[0] # 0 for 'free' 1 for 'ref'
...
for username in usernames:
    ...
    for emotion in emotions:
        feature_data_train = load_image_train_data_loso(
            work_directory, actuator, username, usernames, emotion, task, posture, num_instances
        )
        all_emo_data_train.append([feature_data_train, emotion])

        feature_data_test = load_image_test_data_loso(
            work_directory, actuator, username, usernames, emotion, task, posture, num_instances
        )
        all_emo_data_test.append([feature_data_test, emotion])

    # Combine data from all emos
    x_train, y_train = combine_all_emo_data(all_emo_data_train, emo_indices)
    print(f"Training data loaded with shape X: {x_train.shape}, Y: {y_train.shape}")
    x_test, y_test = combine_all_emo_data(all_emo_data_test, emo_indices)
    print(f"Testing data loaded with shape X: {x_test.shape}, Y: {y_test.shape}")

    # Apply global normalization to all data points collectively
    # X_train = normalize_all_data(X_train, method='zscore')  # Use 'minmax' for [0, 1] range
    ...
    # Save model
    model_save_path = os.path.join(work_directory, f"segments/{actuator}/{task}_{posture}_loso_cnn.pt")
...
plt.show()
plt.savefig(f"results/{task}_{posture}_loso_cnn_confusion_matrix.png")